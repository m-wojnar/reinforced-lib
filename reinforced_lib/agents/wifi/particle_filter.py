...
class ParticleFilter(BaseAgent):
    ...
    def __init__(
        ...
        self.init = jax.jit(
            partial(
                self.init,
                min_position=min_snr - initial_power,
                max_position=max_snr - initial_power,
                particles_num=particles_num
            )
        )
        self.update = jax.jit(partial(self.update, scale=scale))
    ...
    def update(
        ...
        success_prob_func = jax.vmap(ParticleFilter._success_probability, in_axes=[None, 0])
    ...
    def sample(
        ...
        success_prob_func = jax.vmap(ParticleFilter._success_probability, in_axes=[0, None])
