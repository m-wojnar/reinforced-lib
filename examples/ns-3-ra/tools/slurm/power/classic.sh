...
./ns3.38-ra-sim-optimized --wifiManager="$MANAGER" --wifiManagerName="$MANAGER_NAME" --deltaPower="$DELTA" --intervalPower="$INTERVAL" --velocity="$VELOCITY" --initialPosition="$START_POS" --simulationTime="56" --warmupTime="$WARMUP_TIME" --logEvery="0.5" --lossModel="$LOSS_MODEL" --RngRun="$SEED" --csvPath="$CSV_PATH"
