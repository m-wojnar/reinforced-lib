...
./ns3.38-ra-sim-optimized --mobilityModel="RWPM" --wifiManager="$MANAGER" --wifiManagerName="$MANAGER_NAME" --nodeSpeed="$NODE_SPEED" --nWifi="$N_WIFI" --simulationTime="$SIM_TIME" --warmupTime="$WARMUP_TIME" --logEvery="$SIM_TIME" --lossModel="$LOSS_MODEL" --RngRun="$SEED" --csvPath="$CSV_PATH"