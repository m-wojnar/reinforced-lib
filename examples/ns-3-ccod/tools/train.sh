...
for (( i = 1; i <= NUM_REPS; i += 1)); do
  ...
  python3 main.py --ns3Path="$NS3_DIR" --agent="$AGENT" --agentType="$AGENT_TYPE" --nWifi="$N_WIFI" --scenario="$SCENARIO" --pythonSeed="$SEED" --rng="$SEED" --loadPath="$LOAD_PATH" --savePath="$SAVE_PATH"
